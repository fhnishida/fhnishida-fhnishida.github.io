...
- [Se��o 2.2 de Heiss (2020)](http://www.urfie.net/read/index.html#page/98)

- Podemos usar a fun��o `coef()` para extrairmos um vetor com as estimativas da regress�o, {{<math>}}$\hat{\boldsymbol{\beta}}${{</math>}}
...
- Al�m disso, podemos extrair os valores ajustados/preditos ({{<math>}}$\hat{\boldsymbol{y}}${{</math>}}) e os res�duos ({{<math>}}$\hat{\boldsymbol{\varepsilon}} = \boldsymbol{y} - \hat{\boldsymbol{y}}${{</math>}}) usando, respectivamente, as fun��es `fitted()` e `resid()`:
...
- Podemos extrair informa��es mais detalhadas do objeto resultante da regress�o (`lm()`) por meio da fun��o `summary()`:

```r
summary(GPAres) # informa��es detalhadas da regress�o
```
...
- Defina o vetor-coluna de par�metros, {{<math>}}$\boldsymbol{\beta}${{</math>}}, e o vetor-linha das vari�veis explicativas da observa��o {{<math>}}$i${{</math>}}, {{<math>}}$\boldsymbol{x}_i${{</math>}} (min�sculo):
...
- Rejeitamos a hip�tese nula, neste caso, quando {{<math>}}$a_k${{</math>}} estiver fora do intervalo de confian�a.
...
- `diff_fte`: diferen�a de n� total de empregados entre fev/1992 e nov/1992
...
- Isto � comum nas bases de dados, j� que n�meros consomem menos espa�o de armazenamento do que texto.
...
{{< cta cta_text="?Y'? Seguir para Revis�o | Testes de Hip�tese" cta_link="../sec2" >}}
