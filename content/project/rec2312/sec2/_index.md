...
$$ H_0: \ \beta_k = a_k \tag{4.1} $$
tal que {{<math>}}$a_k${{</math>}} é uma constante, e {{<math>}}$k${{</math>}} é um dos {{<math>}}$K+1${{</math>}} parâmetros estimados.

- A hipótese alternativa para teste bicaudal é dada por
$$ H_1: \ \beta_k \neq a_k \tag{4.2} $$
enquanto, para teste unicaudal, é
$$ H_1: \ \beta_k > a_k \qquad \text{ou} \qquad H_1: \ \beta_k < a_k \tag{4.3} $$

- Estas hipóteses podem ser convenientemente testas pelo test _t_:
$$ t = \frac{\hat{\beta}_k - a_k}{\text{se}(\hat{\beta}_k)} \tag{4.4} $$

- Frequentemente, realizamos teste bicaudal com {{<math>}}$a_k=0${{</math>}} para testar se a estimativa {{<math>}}$\hat{\beta}_k${{</math>}} é estatisticamente significante, ou seja, se a variável independente tem efeito significante sobre a variável dependente (estatisticamente diferente de zero):
