...
title: Revis�o de Programa��o em R
weight: 1
output: md_document
type: book
---
...
x < 0 | x^2 > 100 # TRUE se UMA das express�es for TRUE
```
...
```r
x < 0 & x^2 > 100 # TRUE se TODAS express�es for TRUE
```
...
- **N�veis 1 e 2**: s�o usados para JUNTAR express�es l�gicas
...
- Podemos criar um vetor usando a fun��o `c()` e incluindo os valores separados por `,`:
...
### Matrizes
Matrizes s�o vetores (e, portanto, possuem elementos de mesma classe) com atributo de _dimens�o_ (n� linhas por n� colunas). Uma matriz pode ser criada usando a fun��o `matrix()`:
...
?? poss�vel construir uma matriz "preenchida" incluindo um vetor com (n� linhas {{<math>}}$\times${{</math>}} n� colunas) elementos. 
...
Podemos criar matrizes identidade facilmente utilizando a fun��o `diag()` e informando o n� de elementos na diagonal principal da matriz:
...
- Ao utilizar as opera��es matem�ticas convencionais em vetores, cada elemento � operacionalizado com o elemento na mesma posi��o do outro vetor
...
x + y # soma de cada elemento na mesma posi��o
```
...
```r
x * y # multiplica��o de cada elemento na mesma posi��o
```
...
```r
x / y # divis�o de cada elemento na mesma posi��o
```
...
```r
X + Y # Soma de elementos na mesma posi��o
```
...
```r
X * Y # Multiplica��o de elementos na mesma posi��o
```
...
- E, � claro, d� para fazer opera��es entre vetor e matriz:
...
- Note que, caso voc� n�o tenha definido o diret�rio de trabalho ou queria fazer download diretamente da internet, � necess�rio informar o caminho/endere�o inteiro da base de dados que voc� quer importar:
...
H� algumas bases de dados j� presentes no pacote `base` (j� carregado automaticamente) do R. Para list�-las, pode-se usar a fun��o `data()`:
```r
data()
```

```r
## Data sets in package ??~datasets??T:
## 
...
?? poss�vel acess�-las apenas escrevendo seu nome. Vamos ver as 6 primeras linhas (fun��o `head()`) de uma base de dados listada:
...
Al�m disso, � poss�vel baixar um pacote e usar uma de suas bases de dados usando a fun��o `data()`. Abaixo, vamos instalar o pacote `wooldridge` e carregar a de suas bases de dados `gpa1`:
...
- Normalmente, preenchemos essa nova vari�vel a partir da informa��o de outras vari�veis pr�-existentes.
- Usando a base de dados `mtcars`, vamos criar as vari�veis:
  ...
  - `mpg_am`: intera��o (multiplica��o) da vari�vel `mpg` com a `am`, que � uma vari�vel _dummy_ (valores s�o apenas 0 e 1)
...
mtcars$mpg_am = mtcars$mpg * mtcars$am # intera��o entre mpg e am
...
{{< cta cta_text="?Y'? Seguir para M�nimos Quadrados Ordin�rios (MQO)" cta_link="../sec2" >}}
